...
setup_user_paging()
{
  //2013011429: lec7-spoc challenge-part2
}
  
setup_kernel_paging()
{
  //2013011429: lec7-spoc challenge-part1
  uint i,j,k;
  ...
  for (i=0, j=KERSTART; i<16,j<KEREND; i++, j+=PAGE*1024){
	  pg_dir[j>>22] = (uint)pg_tbl[i] | PTE_P | PTE_W;
	  for (k=0;k<1024;k++){
		  pg_tbl[i][k] = (k<<12) | PTE_P | PTE_W;	//每一页有1024，32bit中后12位为保留位和标记位
	  }
  }
